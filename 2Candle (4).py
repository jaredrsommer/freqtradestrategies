...
class Candle2(IStrategy):
    ...
    u_window_size = IntParameter(80, 150, default=90, space='buy', optimize=True, load=True)
    l_window_size = IntParameter(20, 40, default=30, space='buy', optimize=True, load=True)
    sr_length = IntParameter(24, 75, default=50, space="buy")
    sr_shift = IntParameter(24, 75, default=50, space="buy")
    fast_rsi = IntParameter(4, 10, default=10, space="buy")
    ...
    rsi_threshold = DecimalParameter(30.0, 70.0, default=65.0, decimals=1, space="buy")
    bull_rsi_limit = DecimalParameter(65.0, 85.0, default=75.0, decimals=1, space="buy")
    bull_4h_threshold = DecimalParameter(60.0, 80.0, default=70.0, decimals=1, space="buy")
    bear_rsi_threshold = DecimalParameter(40.0, 70.0, default=70.0, decimals=1, space="sell")
    sell_threshold = DecimalParameter(3.0, 4.0, default=4.0, decimals=1, space="sell")

    # Short Limits
    buys_threshold = DecimalParameter(3.0, 5.0, default=4.0, decimals=1, space="buy")
    rsis_threshold = DecimalParameter(30.0, 70.0, default=65.0, decimals=1, space="buy")
    bear_rsi_limit = DecimalParameter(65.0, 85.0, default=75.0, decimals=1, space="buy")
    bear_4h_threshold = DecimalParameter(60.0, 80.0, default=70.0, decimals=1, space="buy")
    bull_rsi_threshold = DecimalParameter(40.0, 70.0, default=70.0, decimals=1, space="sell")
    sells_threshold = DecimalParameter(3.0, 5.0, default=4.0, decimals=1, space="sell")

    # Long Condition Selection
    ...
    # Short Condition Selection
    # if can_short == True:
    use_1s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_2s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_3s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_4s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_5s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_6s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    use_7s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_8s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_9s = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_10s = BooleanParameter(default=True, space="buy", optimize=True, load=True)

    use_11s = BooleanParameter(default=True, space="sell", optimize=True, load=True)
    use_12s = BooleanParameter(default=True, space="sell", optimize=True, load=True)
    use_13s = BooleanParameter(default=True, space="sell", optimize=True, load=True)
    # use_14 = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_15 = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_16 = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    # use_17 = BooleanParameter(default=True, space="buy", optimize=True, load=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (dataframe['pattern_avg'] <= self.buys_threshold.value) &
                (dataframe['pattern_avg'].shift() > 0) &
                (self.use_1s.value == True) &
                (dataframe['rsi_4h'] > self.rsis_threshold.value) &
                (dataframe['rsi'] < dataframe['rsi_4h'])
            ),
            ['enter_short', 'enter_tag']] = (1, 'Pattern and RSI - Short')

        dataframe.loc[
            (
                (dataframe['pattern_avg'] <= self.buy_threshold.value) &
                (dataframe['pattern_avg'].shift() > 0) &
                (self.use_2s.value == True) &
                (dataframe['CO2_4h'] < dataframe['support']) &
                (dataframe['CO2_4h'].shift() > dataframe['support'].shift()) & 
                (dataframe['rsi_4h'] < self.rsis_threshold.value) 
            ),
            ['enter_short', 'enter_tag']] = (1, 'Support Cross - Short')

        dataframe.loc[
            (
                (dataframe['pattern_avg'] <= self.buys_threshold.value) &
                (self.use_3s.value == True) &
                (dataframe['CO2_4h'] < dataframe['resistance']) &
                (dataframe['CO2_4h'].shift() > dataframe['resistance'].shift()) 
            ),
            ['enter_short', 'enter_tag']] = (1, 'Resistance Cross - Short')

        dataframe.loc[
            (
                # High close bull candle (pattern 4: most bullish)
                (dataframe['pattern_avg'] <= self.buys_threshold.value) &
                (self.use_4s.value == True) &
                (dataframe['CO2_4h'] > dataframe['resistance']) &
                (dataframe['CO2_4h'].shift(4) > dataframe['CO2_4h']) &
                (dataframe['rsi'] < dataframe['rsi_4h'])

            ),
            ['enter_short', 'enter_tag']] = (1, 'Above Resistance 4h push down - Short')


        dataframe.loc[
            (
                (dataframe['CO2_4h'] > dataframe['resistance']) &
                (self.use_5s.value == True) &
                (dataframe['rsi_4h'] > 90) &
                (dataframe['rsi'] < dataframe['rsi_4h'])
            ),
            ['enter_short', 'enter_tag']] = (1, '4h Extreme RSI - Short')

        dataframe.loc[
            (
                (dataframe['rsi_4h'] < 50) &  # Proxy for uptrend
                (dataframe['pattern_4h'] <= 3) &  # Strong bull pattern on 4h
                (self.use_6s.value == True) &
                (dataframe['close'] < dataframe['inflection']) &
                (dataframe['close'].shift() > dataframe['inflection']) &  # Cross above midpoint
                (dataframe['rsi'] < dataframe['rsi_4h'])  # Local RSI strength
            ),
            ['enter_short', 'enter_tag']] = (1, 'Downtrend Inflection Cross - Short')

        dataframe.loc[
            (
                (dataframe['bull_bear'] == -1) &
                (dataframe['pattern_avg'] < self.buys_threshold.value + 1) &
                (self.use_7s.value == True) &
                (dataframe['rsi_4h'] < self.bear_4h_threshold.value) &
                (dataframe['rsi'] > self.bear_rsi_limit.value) &
                (dataframe['close'] < dataframe['close'].shift())
            ),
            ['enter_long', 'enter_tag']] = (1, 'Trending Bear Entry - Short')
...
#   },
#   "ft_stratparam_v": 1,
#   "export_time": "2025-07-22 02:55:05.025738+00:00"