...
class HurstCycle7(IStrategy):
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        dataframe = do_indicators(self, dataframe, metadata)
        dataframe['closePos_h2_Sum'] = (dataframe['closePos_h2'] + dataframe['closePos_h2_4h']) / 2
        dataframe['closePos_h2_SumSmoo'] = dataframe['closePos_h2_Sum'].rolling(3).mean()
        # dataframe['closePos_cp_Sum'] = (dataframe['closePos_cp'] + dataframe['closePos_cp_4h']) / 2
        # dataframe['closePos_trend'] = (
        #     (dataframe['closePos_h2_Sum'] + dataframe['closePos_cp_Sum']) / 3
        #     )
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            (dataframe['close'] > dataframe['vtl_up_4h']) &
            (dataframe['close'].shift() < dataframe['vtl_up_4h'].shift()),
            'enter_long'] = 1
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            # (dataframe['close'].shift() > dataframe['vtl_down'].shift()) &
            # (dataframe['close'] < dataframe['vtl_down']), 
            (dataframe['synergy_sell_4h'] == 1), 
            'exit_long'] = 1

        dataframe.loc[
            (dataframe['close'].shift() > dataframe['vtl_down_4h'].shift()) &
            (dataframe['close'] < dataframe['vtl_down_4h']), 
            # (dataframe['synergy_sell'] == 1), 
            'exit_long'] = 1 
        return dataframe
