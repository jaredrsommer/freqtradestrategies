...
class AlexBTK_CT(IStrategy):
    ...
    Key improvements:
      ...
      - NEW: Rolling Market correlation filters for better entry timing
    """
    ...
    plot_config = {
        ...
        "maxima_sort_threshold": {
          "color": "#9d4227",
          "type": "line"
        },
        "minima_sort_threshold": {
          "color": "#62aa10",
          "type": "line"
        },
        "enter_tag": {
    ...
    def calculate_rolling_murrey_math_levels_optimized(self, df: pd.DataFrame, window_size: int) -> Dict[str, pd.Series]:
        ...
        for i in range(0, len(df), calculation_step):
            ...
            levels = AlexBTK_CT._calculate_mml_core(mn_period, mx_period, mx_period, mn_period, mml_c1, mml_c2)
    ...
    def calculate_market_correlation(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if is_futures:
            btc_pairs_to_try.extend([
                f"BTC/{quote_currency}:{settlement_currency}",
                f"BTC/USDT:USDT",
                f"BTC/BUSD:BUSD",
            ])
        ...
        correlation_period = min(24, len(dataframe) // 4)  
        ...
        merged['btc_correlation'] = merged['returns'].rolling(
            window=correlation_period, min_periods=correlation_period // 2
        ).corr(merged['btc_returns'])
        
        dataframe = pd.merge(dataframe, merged[['date', 'btc_correlation']], on='date', how='left')
        ...
        btc_close = btc_dataframe['close'].values
        btc_sma20 = btc_dataframe['btc_sma20'].values
        btc_sma50 = btc_dataframe['btc_sma50'].values
        
        btc_trend = np.where((btc_close > btc_sma20) & (btc_sma20 > btc_sma50), 1, 
                             np.where((btc_close < btc_sma20) & (btc_sma20 < btc_sma50), -1, 0))
        
        dataframe['btc_trend'] = np.nan
        dataframe['btc_close'] = np.nan
        dataframe['btc_sma20'] = np.nan
        dataframe['btc_sma50'] = np.nan
        
        for i in range(len(dataframe)):
            date = dataframe['date'].iloc[i]
            btc_idx = btc_dataframe[btc_dataframe['date'] == date].index
            
            if len(btc_idx) > 0:
                btc_idx = btc_idx[0]
                if btc_idx < len(btc_trend):
                    dataframe.at[dataframe.index[i], 'btc_trend'] = btc_trend[btc_idx]
                    dataframe.at[dataframe.index[i], 'btc_close'] = btc_close[btc_idx]
                    dataframe.at[dataframe.index[i], 'btc_sma20'] = btc_sma20[btc_idx]
                    dataframe.at[dataframe.index[i], 'btc_sma50'] = btc_sma50[btc_idx]
        
        dataframe['btc_correlation'] = dataframe['btc_correlation'].fillna(0.5)
        dataframe['btc_trend'] = dataframe['btc_trend'].fillna(0)
        dataframe['btc_close'] = dataframe['btc_close'].fillna(dataframe['close'])
        dataframe['btc_sma20'] = dataframe['btc_sma20'].fillna(dataframe['close'])
        dataframe['btc_sma50'] = dataframe['btc_sma50'].fillna(dataframe['close'])
    ...
    def calculate_market_breadth(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        if is_futures:
            ...
            top_pairs = [
                ...
                f"XRP/USDT:{settlement}", f"SUI/USDT:{settlement}"
            ]
        else:
            top_pairs = [
        ...
        for i in range(len(dataframe)):
            ...
            for check_pair in top_pairs:
                try:
                    ...
                    if pair_data.empty or len(pair_data) <= i:
                        continue
                    
                    current_close = pair_data['close'].iloc[i]
                    sma20 = pair_data['close'].rolling(20).mean().iloc[i]
                    
                    if current_close > sma20 * 1.005:
                        bullish_count += 1
                    elif current_close < sma20 * 0.995:
                        bearish_count += 1
                    else:
                        neutral_count += 1
            ...
            market_breadth_data.append(market_breadth)
        
        dataframe['market_breadth'] = market_breadth_data
    ...
    def calculate_market_regime(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        for i in range(len(dataframe)):
            date = dataframe['date'].iloc[i]
            btc_idx = btc_data[btc_data['date'] == date].index
            
            if len(btc_idx) > 0:
                btc_idx = btc_idx[0]
                if btc_idx < len(market_volatility) and btc_idx < len(btc_adx):
                    current_volatility = market_volatility.iloc[btc_idx]
                    current_adx = btc_adx.iloc[btc_idx]
                    ...
                    market_regime_data.append(regime)
                    market_volatility_data.append(current_volatility)
                    market_adx_data.append(current_adx)
                else:
                    market_regime_data.append('unknown')
                    market_volatility_data.append(0.02)
                    market_adx_data.append(25)
            else:
                market_regime_data.append('unknown')
                market_volatility_data.append(0.02)
                market_adx_data.append(25)
        
        dataframe['market_regime'] = market_regime_data
        dataframe['market_volatility'] = market_volatility_data
        dataframe['market_adx'] = market_adx_data
        ...
        for i in range(len(dataframe)):
            ...
            for coin_pair, weight in major_coins.items():
                try:
                    ...
                    if coin_data.empty or len(coin_data) <= i:
                    ...
                    current_price = coin_data['close'].iloc[i]
                    ma_value = coin_data['close'].rolling(self.total_mcap_ma_period.value).mean().iloc[i]
            ...
            mcap_trend_data.append(mcap_trend)
        
        dataframe['mcap_trend'] = mcap_trend_data
